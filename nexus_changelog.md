...
- New: A weather state SLM sets stays set. The game drops a forced weather on its own, and a teleport alone is enough, so SLM keeps putting it back. If another mod keeps overriding it, SLM says so and stops fighting - clear that mod's lock first.
