...
- Note: Twenty-eight strings are format templates rather than concatenations, so word order is the translator's to choose. `TRANSLATING.md` states the rules; the CET font constraint is in it, because CET loads one extra alphabet chosen in its own config and no mod can set it.
