...
- Fix: Clearing a map pin could untrack the player's own waypoint. `UntrackMappin` takes no argument and clears whatever is in the slot, so it now runs only while `GetManuallyTrackedMappinID` reports the slot holds this mod's pin.
...
- Feature: A teleport that does not name a usable weather hands the sky back to the game's cycle, so the previous location's weather does not follow the player around. `Logic.ReleaseWeatherHold` only releases a hold this mod owns - another mod's forced weather is not this mod's to undo - and it covers all three cases: no `env` at all, no weather on the `env`, and a weather that is not installed.
